...
OpenLayers.Renderer.Heatmap = OpenLayers.Class(OpenLayers.Renderer.Canvas, {
    ...
    redraw: function() {
        ...
        if (!this.locked) {
            ...
            var heat = this.canvas.createImageData(this.root.width, this.root.height);
            ...
            for (var x = 0; x < this.root.width; x++) {
                for (var y = 0; y < this.root.height; y++) {
                    ...
                    if (alpha > 0) {
                        ...
                        heat.data[idx] = rgb[0];
                        heat.data[idx+1] = rgb[1];
                        heat.data[idx+2] = rgb[2];
                        heat.data[idx+3] = alpha;
                    }
                }
            }
            this.canvas.putImageData(heat, 0, 0);
        }
    },
